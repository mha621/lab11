...
public class FamilyTree
{
	...
	private void addLine(String line) throws TreeException
	{
		...
		if (colonIndex < 0)
			throw new TreeException ("Index out of bounds");// a TreeException with a useful message
		...
		else
		{
			...
			if (parentNode == null) {
				throw new TreeException("Error: Parent node not found.");
			}
		...
		for (int i = 0; i < childrenArray.length; i++) {
			TreeNode newChild = new TreeNode(childrenArray[i]);
	...
	TreeNode getMostRecentCommonAncestor(String name1, String name2) throws TreeException
	{
		...
		if (node1 == null)
			throw new TreeException("No node with this name.");// Throw a TreeException with a useful message
		TreeNode node2 = root.getNodeWithName(name2);	// node whose name is name2
		if (node2 == null)
			throw new TreeException("No node with this name."); // Throw TreeException with a useful message
